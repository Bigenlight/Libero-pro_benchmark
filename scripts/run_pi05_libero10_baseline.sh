...
# ── Pi0.5 LIBERO-10 Baseline (no perturbation) evaluation launcher ──
# Runs libero10_baseline_eval.py inside the libero-pro container,
# hitting the pi05 HTTP server running on the host.
# No OOD swap perturbation — evaluates the standard LIBERO-10 tasks.
#
# Usage:
#   ./scripts/run_pi05_libero10_baseline.sh
#   ./scripts/run_pi05_libero10_baseline.sh --vla-url http://localhost:8400 --gpu 3
#   ./scripts/run_pi05_libero10_baseline.sh --tasks "task1,task2" --max-trials 10
#
# Default GPU is 3 (GPU 1 is reserved for concurrent swap eval).
...
usage() {
    cat <<USAGE
Usage: ./scripts/run_pi05_libero10_baseline.sh [OPTIONS]

Options:
  --vla-url <url>         Pi0.5 server URL (default: \$VLA_SERVER_URL or http://localhost:8400)
  --gpu <id>              CUDA device index (default: \$CUDA_DEV or 3). GPU 0 is reserved.
  --tasks <csv>           Comma-separated task names to run (default: all tasks)
  --max-trials <N>        Maximum number of trials per task (default: 10)
...
echo "=== run_pi05_libero10_baseline.sh ==="
echo "  IMAGE        : $IMAGE"
...
# 3. OOD data dir check — warning only (baseline doesn't strictly need it)
if [ ! -d "$OOD_DATA_DIR/bddl_files/libero_10_swap" ]; then
    echo "WARN: $OOD_DATA_DIR/bddl_files/libero_10_swap not found — baseline doesn't need it, continuing."
fi
...
# ── Run eval ──
python scripts/libero10_baseline_eval.py \
    --vla-url \"${VLA_URL}\" \
    ${TASKS_FLAG} \
    ...
    --output-dir /workspace/LIBERO-PRO/test_outputs/pi05_libero10_baseline
"

echo ""
echo "Outputs: $OUTPUT_DIR/pi05_libero10_baseline/"
